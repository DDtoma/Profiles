...
read -p "enter your useanme：" usrnm

until [[ "$(echo "${usrnm}" | cut -c 1)" =~ ^[[:lower:]] ]]
do
      	echo -e "\n用户名必须以小写英文字母开头！"
	read -p "enter your new usename：" usrnm
done
#这里使用until来智能判断用户名是否合法，感谢@鼠标乱飘 提供的命令。

read -s -p "enter your password：" usrpasswd
...
echo ""
echo "#install Necessary Component" >> continue.sh
...
cat << EOF
Are you install Yaourt?

EOF

while true
do
	read -n1 -p "enter Y or N：" yaourt
	echo ""
	if [ ${yaourt} = Y ] || [ ${yaourt} = y ];then
		echo "#installing Yaourt" >> continue.sh
		echo "mkdir yaourt" >> continue.sh
		echo "cd yaourt" >> continue.sh
		echo "" >> continue.sh
		
		echo "##Installing dependence：package-query" >> continue.sh
		...
		echo "#don't install Yaourt" >> continue.sh
		break
	fi
done
...
if [ ${choose} == 'emacs' ];then
    git clone https://github.com/syl20bnr/spacemacs.git /home/${usrnm}/.emacs.d
fi
...
请选择您喜欢的播放器，选择smplayerenter 1，VLCenter 2。3,不安装
